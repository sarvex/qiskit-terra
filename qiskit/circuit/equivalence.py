...
def _raise_if_param_mismatch(gate_params, circuit_parameters):
    ...
    if set(gate_parameters) != circuit_parameters:
        raise CircuitError(
            f"Cannot add equivalence between circuit and gate of different parameters. Gate params: {gate_parameters}. Circuit params: {circuit_parameters}."
        )
...
def _raise_if_shape_mismatch(gate, circuit):
    if gate.num_qubits != circuit.num_qubits or gate.num_clbits != circuit.num_clbits:
        raise CircuitError(
            f"Cannot add equivalence between circuit and gate of different shapes. Gate: {gate.num_qubits} qubits and {gate.num_clbits} clbits. Circuit: {circuit.num_qubits} qubits and {circuit.num_clbits} clbits."
        )
